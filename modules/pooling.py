...
def preprocess(A_list, X_list, coarsening_levels, pool):
    mats = pooling_matrices(A_list, coarsening_levels, pool)
    if pool=='graclus':
        ...
        for i, perm_i in enumerate(perm_list):
            X_list[i] = perm_data(X_list[i].T, perm_i).T
...
def pooling_matrices(A_list, coarsening_levels, pool, sparsify=True, sparse_thresh=1e-2):
    ...
    :param decimation_pool: if True, use decimation instead of clustering as
    pooling strategy.
    :param last_mat_ignored: if True, assume that the last decimation matrix is
    not used to reduce the graph in the network (needed for average pooling).
    Set to -1 if the the last decimation matrix of the pyramid is used, set to -2 otherwise.
    :return:
...
def decimation_matrices(levels, A_list, indices):
    ...
    for i in range(max(levels) + 1):
        ...
        # print(S_prev.shape, S_i.shape, A_list[i].shape, "----",indices[i])
...
################################################################################
# METHODS FOR GRAPH COARSENING WITH GRACLUS 
################################################################################
def graclus_pooling_matrices(graphs):
    ...
    # is just for having the same number of pooling mats and coarsened adjs
